...
struct LeaderboardCalculator {
    ...
    static func calculate(from votes: [Vote], contestants: [Contestant]) -> Leaderboard {
        let contestantsMap: [Int: Contestant] = Dictionary(uniqueKeysWithValues: contestants.map { ($0.id, $0) })
        var scores: [Int: Int] = Dictionary(uniqueKeysWithValues: contestants.map { ($0.id, 0) })
